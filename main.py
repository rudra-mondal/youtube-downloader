...
class DownloaderAppLogic:
    ...
    def _update_ui(self, control, **kwargs):
        ...
        try:
            ...
            print(f"Error updating control {type(control)} ({control.uid}): {e}")
    ...
    def set_ui_controls(self, controls: dict):
        ...
        if "metadata_container" in controls:
            self.metadata_container = controls["metadata_container"]
        if "metadata_text" in controls:
            self.metadata_text = controls["metadata_text"]
    ...
    def _update_metadata_content(self, widget):
        if self.metadata_container:
            ...
            print("Error: metadata_container reference not set.")
    ...
    def fetch_content_info(self, e=None):
        ...
        if not url:
            ...
            self._update_metadata_content(error_text)
            play_sound(no_url_sound)
            return
    ...
    def detect_video_platform(self, link): # Unchanged
        fb_watch_pattern = r"^(https?://)?fb\.watch/.*"
        pin_it_pattern = r"^(https?://)?pin\.it/.*"
        if re.match(pin_it_pattern, link, re.IGNORECASE):
            return "pinterest"
        if re.match(fb_watch_pattern, link, re.IGNORECASE):
            return "facebook"
        ...
        for p in pinterest_patterns:
            if re.match(p,link,re.IGNORECASE):
                return "pinterest"
    ...
    def fetch_content_info_thread(self, url): # Unchanged
        ...
        if not self.current_platform:
            ...
            self._update_metadata_content(error_text)
            self._update_ui(self.status_text, value="")
            self._update_ui(self.fetch_button, disabled=False)
            play_sound(no_url_sound)
            return
        platform_name = self.current_platform.capitalize()
        self._update_ui(self.status_text, value=f"Getting {platform_name} info...", color=WARNING_COLOR)
        play_sound(fetch_start_sound)
        try:
            ydl_opts_fetch = {'quiet': True, 'skip_download': True, 'forcejson': False, 'noplaylist': True, 'logger': YtDlpLogger()}
            if self.current_platform == "facebook":
                cookie_path = resource_path("cookies/facebook_cookies.txt")
                ydl_opts_fetch['cookiefile'] = cookie_path if os.path.exists(cookie_path) else None
            with yt_dlp.YoutubeDL(ydl_opts_fetch) as ydl:
                try:
                    self.content_data = ydl.extract_info(url, download=False)
                except yt_dlp.utils.DownloadError as e:
                    error_msg = f"{e}"
                    error_msg += "\nTry adding cookies." if "login required" in error_msg.lower() else ""
                    error_msg = "Content unavailable or private." if "unavailable" in error_msg.lower() else error_msg
                    error_widget = Text(f"Error fetching info: {error_msg}", color=ERROR_COLOR, weight=ft.FontWeight.W_500, text_align=ft.TextAlign.CENTER)
                    self._update_metadata_content(error_widget)
                    self._update_ui(self.status_text, value="")
                    self._update_ui(self.fetch_button, disabled=False)
                    play_sound(error_sound)
                    return
            if not self.content_data:
                raise ValueError("Failed to extract content data.")
            if self.current_platform == "pinterest":
                formats = self.content_data.get('formats', [])
                self.is_pinterest_video = any(fmt.get('vcodec') not in (None, 'none') for fmt in formats)
            thumbnail_url = self.content_data.get('thumbnail')
            if self.current_platform == "pinterest" and not self.is_pinterest_video:
                image_url = self.content_data.get('url')
                thumbnail_url = image_url if image_url and ('/170x/' not in (thumbnail_url or '') and '170x.' not in (thumbnail_url or '')) else thumbnail_url
            self._display_thumbnail(thumbnail_url or self.content_data.get('url'))
            title = self.content_data.get('title', 'No Title Found')
            uploader = self.content_data.get('uploader', 'Unknown Uploader')
            duration_raw = self.content_data.get('duration')
            duration = self.content_data.get('duration_string', None)
            if not duration and isinstance(duration_raw, (int, float)):
                td = time.gmtime(duration_raw)
                duration = time.strftime('%H:%M:%S' if duration_raw >= 3600 else '%M:%S', td)
            duration = duration or "N/A"
            content_type = None
            if self.current_platform == "pinterest":
                content_type = "Video" if self.is_pinterest_video else self.content_data.get('ext', 'Image/GIF').upper()
            ...
            quality_choices = []
            if self.current_platform == "pinterest" and not self.is_pinterest_video:
                original_ext = self.content_data.get('ext', 'file').upper()
                format_name = f"{original_ext}" if original_ext != 'FILE' else "Image/GIF"
                available_formats = [ft.dropdown.Option(format_name)]
                self._update_ui(self.quality_dropdown, options=[ft.dropdown.Option("Original")], value="Original", disabled=True)
                self.download_quality = "Original"
            else:
                formats = self.content_data.get('formats', [])
                quality_choices_mp4 = sorted(list(set(f"{int(fmt.get('height'))}p" for fmt in formats if fmt.get("vcodec") not in [None, 'none'] and fmt.get("height") and fmt.get('ext') == 'mp4')), key=lambda x: int(x[:-1]), reverse=True)
                if not quality_choices_mp4:
                    quality_choices_mp4 = sorted(list(set(f"{int(fmt.get('height'))}p" for fmt in formats if fmt.get("vcodec") not in [None, 'none'] and fmt.get("height"))), key=lambda x: int(x[:-1]), reverse=True)
                ...
                if quality_choices:
                     self._update_ui(self.quality_dropdown, options=quality_choices, disabled=False)
                     default_quality = next((q.key for q in quality_choices if "1080" in q.key), quality_choices[0].key if quality_choices else "N/A")
                     self._update_ui(self.quality_dropdown, value=default_quality)
                     self.download_quality = default_quality
                else:
                    self._update_ui(self.quality_dropdown, options=[ft.dropdown.Option("N/A")], value="N/A", disabled=True)
                    self.download_quality = None
            self._update_ui(self.format_dropdown, options=available_formats, disabled=False)
            default_format = available_formats[0].key
            self._update_ui(self.format_dropdown, value=default_format)
            self.set_format(None)
            self._finalize_fetch(success=True)
        except Exception as e:
             error_widget = Text(f"Error processing info: {e}", color=ERROR_COLOR, weight=ft.FontWeight.W_500, text_align=ft.TextAlign.CENTER)
             self._update_metadata_content(error_widget)
             self._update_ui(self.status_text, value="")
             self._update_ui(self.fetch_button, disabled=False)
             play_sound(error_sound)
             print(f"Detailed fetch processing error: {type(e).__name__}: {e}")

    def _display_thumbnail(self, thumbnail_url): # Unchanged
        if not thumbnail_url:
            self._update_ui(self.thumbnail_image, src_base64=self.placeholder_base64, tooltip="No thumbnail URL")
            return
        try:
            response = requests.get(thumbnail_url, timeout=10)
            response.raise_for_status()
            img_data = BytesIO(response.content)
            thumbnail = PILImage.open(img_data)
    ...
    def _finalize_fetch(self, success=True, error_message=None): # Unchanged
        if success:
            self._update_ui(self.status_text, value="Ready to Download!", color=SUCCESS_COLOR)
            self._update_ui(self.download_button, disabled=False)
            play_sound(fetch_complete_sound)
        else:
            ...
            if error_message:
                 if isinstance(error_message, str):
                    if "HTTP Error 404" in error_message:
                        display_error = "Content not found (404 Error)."
                    elif "private video" in error_message.lower():
                        display_error = "Cannot access private content."
                    elif "login required" in error_message.lower():
                        display_error = "Login required. Check cookies if needed."
                    elif "urlopen error" in error_message.lower():
                        display_error = "Network error. Please check connection."
                    elif "Unsupported URL" in error_message:
                        display_error = "This specific URL format might not be supported."
                    else:
                        display_error = f"Fetch Error: {error_message[:120]}"
                 else:
                     display_error = f"Fetch Error: {str(error_message)[:120]}"
            error_text_widget = Text(display_error, color=ERROR_COLOR, weight=ft.FontWeight.W_500, text_align=ft.TextAlign.CENTER)
            self._update_metadata_content(error_text_widget)
            play_sound(error_sound)
        self._update_ui(self.fetch_button, disabled=False)
    ...
    def _create_metadata_display(self, platform, title, uploader, duration, content_type=None): # Unchanged
        ...
        if duration and duration != 'N/A':
            controls.append(TextSpan(f"  •  Duration: {duration}", TextStyle(size=13, color=TEXT_COLOR_SECONDARY)))
    ...
    def set_format(self, e): # Unchanged
        ...
        if not choice:
            return
        is_video = "Video (MP4)" in choice
        is_audio = "Audio (MP3)" in choice
        is_pinterest_image = self.current_platform == "pinterest" and not self.is_pinterest_video
        if is_audio:
            self.download_format = "mp3"
            self._update_ui(self.quality_dropdown, disabled=True, value="N/A")
            self.download_quality = None
        elif is_video:
            self.download_format = "mp4"
            quality_options = self.quality_dropdown.options if self.quality_dropdown else []
            quality_available = quality_options and quality_options[0].key != "N/A"
            self._update_ui(self.quality_dropdown, disabled=not quality_available)
            if quality_available and (not self.download_quality or self.download_quality == "N/A"):
                self.download_quality = quality_options[0].key
            if not quality_available:
                self.download_quality = None
                self._update_ui(self.quality_dropdown, value="N/A")
            else:
                self._update_ui(self.quality_dropdown, value=self.download_quality)
        elif is_pinterest_image:
             self.download_format = self.content_data.get('ext', 'file')
             self._update_ui(self.quality_dropdown, disabled=True, value="Original")
             self.download_quality = "Original"
        else:
            self.download_format = "mp4"
            self._update_ui(self.quality_dropdown, disabled=False if self.quality_dropdown.options and self.quality_dropdown.options[0].key != "N/A" else True)

    def choose_path(self, e): # Unchanged
        if self.file_picker:
            self.file_picker.get_directory_path(initial_directory=self.download_path, dialog_title="Select Download Folder")
    ...
    def update_path_display(self): # Unchanged
        if self.path_display_text:
            max_display_len=40
            display_text=self.download_path
    ...
    def start_download(self, e=None): # Unchanged
        if not self.content_data:
            error_text = Text("Please fetch content details first!", color=ERROR_COLOR, weight=ft.FontWeight.W_500, text_align=ft.TextAlign.CENTER)
            self._update_metadata_content(error_text)
            play_sound(no_url_sound)
            return
        url = self.url_entry.value.strip() if self.url_entry else ""
        if not url:
            error_text = Text("URL seems to be missing!", color=ERROR_COLOR, weight=ft.FontWeight.W_500, text_align=ft.TextAlign.CENTER)
            self._update_metadata_content(error_text)
            play_sound(no_url_sound)
            return
        is_video_download = (self.current_platform != "pinterest" or self.is_pinterest_video)
        if is_video_download and self.download_format == "mp4" and (not self.download_quality or self.download_quality == "N/A"):
            error_text = Text("Please select a valid video quality for MP4 download!", color=ERROR_COLOR, weight=ft.FontWeight.W_500, text_align=ft.TextAlign.CENTER)
            self._update_metadata_content(error_text)
            play_sound(no_url_sound)
            return
        self._update_ui(self.progress_bar, value=None, color=ACCENT_COLOR)
        self._update_ui(self.time_label, value="")
        self._update_ui(self.status_text, value="Preparing download...", color=INFO_COLOR)
        self._update_ui(self.download_button, disabled=True)
        self._update_ui(self.fetch_button, disabled=True)
        self.start_time = time.time()
        self.last_downloaded_bytes, self.last_time, self.last_speed = 0, self.start_time, None
        play_sound(download_start_sound)
    ...
    def _execute_download_and_convert(self, url): # Unchanged
        if not self.content_data:
            self._finalize_download(success=False, message="Error: Content data missing.")
            return
        if not self.current_platform:
            self._finalize_download(success=False, message="Error: Platform not identified.")
            return
        ...
        try:
            title = self.content_data.get('title', 'downloaded_media')
            title = self.sanitize_filename(title)
            output_template = os.path.join(self.download_path, f'{title}.%(ext)s')
            if is_pinterest_image_gif:
                ...
                quality_spec = f"[height<={self.download_quality[:-1]}]" if self.download_quality and 'p' in self.download_quality else ""
                format_selection = f'bestvideo{quality_spec}+bestaudio/bestvideo{quality_spec}/best{quality_spec}/best'
                merge_format = 'mkv'
            ...
            if self.current_platform == "facebook":
                cookie_path = resource_path("cookies/facebook_cookies.txt")
                ydl_opts['cookiefile'] = cookie_path if os.path.exists(cookie_path) else None
            self._update_ui(self.status_text, value="Downloading...", color=INFO_COLOR)
            ...
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                try:
                    ...
                    if not downloaded_file_path or not os.path.exists(downloaded_file_path):
                         expected_ext = info_dict.get('ext', 'tmp')
                         expected_ext = merge_format if merge_format and expected_ext != merge_format else expected_ext
                         guessed_path = os.path.splitext(output_template % {'ext': 'dummy'})[0] + '.' + expected_ext
                         if os.path.exists(guessed_path):
                             downloaded_file_path = guessed_path
                             print(f"Warning: prepare_filename unreliable, using guessed path: {downloaded_file_path}")
                         else:
                             raise yt_dlp.utils.DownloadError(f"Download finished, but cannot locate output file. Info: {info_dict.get('filepath', 'N/A')}")
                except yt_dlp.utils.DownloadError as e:
                    ...
                    return
            if not downloaded_file_path or not os.path.exists(downloaded_file_path):
                self._finalize_download(success=False, message="Error: Output file not found after download.")
                return
            final_filepath = downloaded_file_path
            if needs_conversion:
                try:
                    if self.download_format == "mp3":
                        final_filepath = self._convert_to_mp3(downloaded_file_path)
                    elif self.download_format == "mp4":
                        final_filepath = self._convert_to_mp4(downloaded_file_path)
                except Exception as e:
            ...
            if final_filepath and os.path.exists(final_filepath):
                ...
                self._finalize_download(success=False, message="Error: Final file not found after processing.")
        except Exception as e:
    ...
    def _finalize_download(self, success=True, message=None): # Unchanged
        final_progress = 1.0 if success else (self.progress_bar.value if self.progress_bar.value is not None else 0)
        bar_color = SUCCESS_COLOR if success else ERROR_COLOR
        self._update_ui(self.progress_bar, value=final_progress, color=bar_color)
        self._update_ui(self.status_text, value="")
        self._update_ui(self.time_label, value="")
        if success:
             final_message = message or "Operation successful!"
             success_text_widget = Text(final_message, color=SUCCESS_COLOR, weight=ft.FontWeight.W_500, text_align=ft.TextAlign.CENTER)
             self._update_metadata_content(success_text_widget)
             play_sound(download_complete_sound)
             self.page.run_thread(self._reset_progress_after_delay, 2.5, PRIMARY_COLOR)
        else:
             error_message = message or "Operation failed!"
             error_text_widget = Text(error_message, color=ERROR_COLOR, weight=ft.FontWeight.W_500, text_align=ft.TextAlign.CENTER)
             self._update_metadata_content(error_text_widget)
             play_sound(error_sound)
             self.page.run_thread(self._reset_progress_after_delay, 4.0, PRIMARY_COLOR)
        self._update_ui(self.download_button, disabled=False)
        self._update_ui(self.fetch_button, disabled=False)

    def _reset_progress_after_delay(self, delay_seconds, default_color): # Unchanged
        time.sleep(delay_seconds)
        self._update_ui(self.progress_bar, value=0, color=default_color)
    ...
    def progress_callback(self, d): # Unchanged
        if d['status'] == 'downloading':
            ...
            if total_bytes and total_bytes > 0:
                ...
                self._update_ui(self.progress_bar, value=progress, color=ACCENT_COLOR)
                current_time = time.time()
                time_diff = current_time - (self.last_time or self.start_time)
                if time_diff >= 0.5:
                    bytes_diff = downloaded_bytes - self.last_downloaded_bytes
                    speed = max(0, bytes_diff / time_diff if time_diff > 0 else 0)
                    alpha = 0.3
                    speed = alpha * speed + (1 - alpha) * self.last_speed if self.last_speed is not None else speed
                    self.last_speed = speed
                    remaining_bytes = total_bytes - downloaded_bytes
                    eta = remaining_bytes / speed if speed > 1 else float('inf')
                    def format_size(b):
                        if b is None or b < 0:
                            return "N/A";
                        if b == 0:
                        ...
                        try:
                            s=" BKBMBGBTB"
                            i=min(len(s)//2-1,int(math.floor(math.log(max(1,b),1024))))
                            p=math.pow(1024,i)
                            r=round(b/p,1)
                            return f"{r} {s[i*2:i*2+2]}"
                        except:
                            return f"{b} B"
                    ...
                    self._update_ui(self.status_text, value=f"Downloading... {progress*100:.1f}%", color=INFO_COLOR)
                    self._update_ui(self.time_label, value=f"{sp} • {dn} / {t} • ETA: {et}")
                    self.last_downloaded_bytes, self.last_time = downloaded_bytes, current_time
            else:
                downloaded_str = format_size(downloaded_bytes) if downloaded_bytes else "0 B"
                self._update_ui(self.status_text, value=f"Downloading... ({downloaded_str})", color=INFO_COLOR)
                self._update_ui(self.progress_bar, value=None, color=ACCENT_COLOR)
                self._update_ui(self.time_label, value="Calculating size...")
        elif d['status'] == 'finished':
            self._update_ui(self.progress_bar, value=1.0, color=ACCENT_COLOR)
            self._update_ui(self.time_label, value="")
        elif d['status'] == 'error':
            self._update_ui(self.status_text, value="Download Error", color=ERROR_COLOR)
    ...
    def _convert_to_mp3(self, downloaded_file):
        self._update_ui(self.status_text, value="Converting to MP3...", color=WARNING_COLOR)
        self._update_ui(self.time_label, value="")
        self._update_ui(self.progress_bar, value=0)
        # Use the pre-determined correct path from __init__
        ffmpeg_cmd_path = self.ffmpeg_path
        ffprobe_cmd_path = self.ffprobe_path
        if not os.path.exists(ffmpeg_cmd_path):
            raise FileNotFoundError(f"ffmpeg not found at {ffmpeg_cmd_path}")
        if not os.path.exists(ffprobe_cmd_path):
            raise FileNotFoundError(f"ffprobe not found at {ffprobe_cmd_path}")

        base_name = os.path.splitext(downloaded_file)[0]
        converted_file = os.path.abspath(base_name + ".mp3").replace('\\', '/')
        ...
        while True:
            ...
            if time_match:
                try:
                     if "out_time_ms" in time_match.group(0):
                         current_seconds = int(time_match.group(1)) / 1_000_000
                     else:
        ...
        if process.returncode == 0 and os.path.exists(converted_file):
            ...
            try:
                os.remove(downloaded_file_abs)
                print(f"Removed original: {downloaded_file_abs}")
            except OSError as e:
                print(f"Warning: Could not remove original file {downloaded_file_abs}: {e}")
            return converted_file
        else:
            error_output = process.stdout.read()
            print(f"FFmpeg MP3 conversion failed! Code: {process.returncode}\nOutput:\n{error_output}")
            raise RuntimeError(f"FFmpeg MP3 conversion failed (Code: {process.returncode}).")

    def _convert_to_mp4(self, downloaded_file):
        if not self.download_quality or self.download_quality == "N/A":
            raise ValueError("MP4 conversion requires a valid quality selection.")
        self._update_ui(self.status_text, value="Converting to MP4...", color=WARNING_COLOR)
        self._update_ui(self.time_label, value="")
        self._update_ui(self.progress_bar, value=0)
        # Use the pre-determined correct path from __init__
        ffmpeg_cmd_path = self.ffmpeg_path
        ...
        base_name = os.path.splitext(downloaded_file)[0]
        converted_file = os.path.abspath(base_name + f"_{self.download_quality}.mp4").replace('\\', '/')
        downloaded_file_abs = os.path.abspath(downloaded_file).replace('\\', '/')
        creation_flags = subprocess.CREATE_NO_WINDOW if sys.platform == 'win32' else 0
        duration = self._get_media_duration(downloaded_file_abs) or 1
        ...
        if process.returncode == 0 and os.path.exists(converted_file):
            ...
            error_output = process.stdout.read()
            print(f"FFmpeg MP4 conversion failed! Code: {process.returncode}\nOutput:\n{error_output}")
    ...
    def _get_media_duration(self, filepath): # Now uses self.ffprobe_path
        ...
        try:
             if not os.path.exists(ffprobe_cmd_path):
                 print(f"ffprobe not found at: {ffprobe_cmd_path}")
                 return None
    ...
    def sanitize_filename(self, filename): # Unchanged
        ...
        return sanitized[:100] if sanitized and sanitized.strip('.') != '' else "downloaded_file"
...
if __name__ == "__main__":
    ...
    if not os.path.exists(ffmpeg_path_check):
        print(f"WARNING: {ffmpeg_exe} not found in bin directory ({ffmpeg_path_check}). Conversions may fail.")
    if not os.path.exists(ffprobe_path_check):
        print(f"WARNING: {ffprobe_exe} not found in bin directory ({ffprobe_path_check}).")
